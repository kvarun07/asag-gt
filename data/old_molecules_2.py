...
class DataUnit(torch.utils.data.Dataset):
    def __init__(self, raw_list):
        ...
        self.graph_lists = [ i[:-1] for i in raw_list ]     # tuple of graphs
        self.graph_labels = [ i[-1] for i in raw_list ]     # scores

        self.s_arg0 = [ i[0] for i in raw_list ]
        self.s_arg1 = [ i[1] for i in raw_list ]
        self.s_default = [ i[2] for i in raw_list ]
        self.s_rest = [ i[3] for i in raw_list ]

        self.m_arg0 = [ i[4] for i in raw_list ]
        self.m_arg1 = [ i[5] for i in raw_list ]
        self.m_default = [ i[6] for i in raw_list ]
        self.m_rest = [ i[7] for i in raw_list ]
...
class MoleculeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        with open('data/all-graph-score.pickle',"rb") as data:
            ...
            print(self.train.__len__(), self.val.__len__(), self.test.__len__())

            print("Self.TRAIN = ", self.train)
            print(self.train.__getitem__(2))
            print(self.train.__len__())
            print(self.train.__getitem__(2)[0][0].nodes())
            print(self.train.__getitem__(2)[0][0].edges())
            print("N_DATA =", self.train.__getitem__(2)[0][0].ndata['feat'].size(), "\n", self.train.__getitem__(2)[0][0].ndata['feat'])
    ...
    def collate(self, samples):
        ...
        # batched_graph = [dgl.batch(graphs[i]) for i in range(len(graphs))]

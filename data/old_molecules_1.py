...
class MoleculeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        data_dir = 'data/molecules/'
        # with open(data_dir+name+'.pkl',"rb") as f:
        with open('data/graph_score.pickle',"rb") as f:
            f = pickle.load(f)
            # self.train = f[0]
            # self.val = f[1]
            # self.test = f[2]
            ...
            print("\nORIGINAL GRAPHS")
            print(f)
            print(f"Data size = {len(f)} graphs!")
            # print(f[0])
            # print(f[0][0])
            # print(f[0][0].nodes(data=True))
            # print(f[0][0].nodes.data)
            # print(f[0][0].edges())
            # print(f[0][0].nodes.data.items())

            print(f[1])
            print(f[1][0])
            print(f[1][0].nodes(data=True))
            print(f[1][0].nodes.data)
            print(f[1][0].edges())
            # print(f[1][0].nodes.data.items())

            print("\nCONVERTED GRAPHS")
            old_list = [(dgl.from_networkx(i[0]), torch.tensor([i[1]])) for i in f]
            g_list = []

            for i in old_list:
              grh = i[0]
              grh.ndata['feat'] = torch.tensor(len(grh.nodes()) * [1])
              # grh.ndata['feat'] = torch.ones(len(grh.nodes()), 1)
              grh.edata['feat'] = torch.ones(grh.number_of_edges(), 1)
              g_list.append((grh, i[1]))

            print(g_list[1])
            print(g_list[1][0].nodes())
            print(g_list[1][0].edges())
            print(f"Data size = {len(g_list)} graphs!")
            ...
            self.train = DataUnit(g_list[:part1])
            self.val = DataUnit(g_list[part1:part2])
            self.test = DataUnit(g_list[part2:])

            # print(len(self.train), len(self.val), len(self.test))
            print(self.train.size, self.val.size, self.test.size)

            # self.train.graph_lists = [ i[0] for i in self.train ]
            # self.val.graph_lists = [ i[0] for i in self.val ]
            # self.test.graph_lists = [ i[0] for i in self.test ]

            # self.train.graph_labels = [ i[1] for i in self.train ]
            # self.val.graph_lists = [ i[1] for i in self.val ]
            # self.test.graph_lists = [ i[1] for i in self.test ]

            print("Self.TRAIN = ", self.train)
            print(self.train.__getitem__(2))
            print(self.train.__len__())
            print(self.train.__getitem__(2)[0].nodes())
            print(self.train.__getitem__(2)[0].edges())
            print("N_DATA =", self.train.__getitem__(2)[0].ndata['feat'].size(), "\n", self.train.__getitem__(2)[0].ndata['feat'])

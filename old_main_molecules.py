...
class MyEnsemble(nn.Module):
    def __init__(self, net_params, model_list):
        ...
        self.MLP_layer_1 = MLPReadout(4*self.out_dim, self.out_dim)
        self.MLP_layer_2 = MLPReadout(2*self.out_dim, 1)

        # Integrate SBERT scores
        self.MLP_layer_3 = MLPReadout(4, 1)
        self.MLP_layer_4 = MLPReadout(2, 1)


        # Create new classifier
        # self.MLP_layer = MLPReadout(2*self.out_dim, self.n_classes) #2 because we have 2 transformers
        
    def forward(self, g, h, e, score_tensor, h_lap_pos_enc=None, h_wl_pos_enc=None):
        ...
        student = tuple(g_out[:4])
        student = torch.cat(student, dim=1)  #dim=1 is the channel dimension

        model = tuple(g_out[4:])
        model = torch.cat(model, dim=1)  #dim=1 is the channel dimension

        student_final = self.MLP_layer_1(student)
        model_final = self.MLP_layer_1(model)

        combined_graph = torch.cat((student_final, model_final), dim=1)
        final_graph = self.MLP_layer_2(combined)
 
        final_text = self.MLP_layer_3(score_tensor)

        combined_all = torch.cat((final_graph, final_text), dim=1)
        final = self.MLP_layer_4(combined_all)

        # x = self.MLP_layer(F.relu(x))
        return final
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    model = MyEnsemble(net_params, model_list)
    model.to(device)
